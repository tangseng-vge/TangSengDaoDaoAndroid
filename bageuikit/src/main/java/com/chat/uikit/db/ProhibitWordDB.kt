...
class ProhibitWordDB private constructor() {
        ...
        try {
            ...
            if (insertCVList.isNotEmpty()) {
                for (cv in insertCVList) {
                    BageBaseApplication.getInstance().dbHelper.insert(table, cv)
                }
            }
            if (updateCVList.isNotEmpty()) {
                for (cv in updateCVList) {
                    ...
                    BageBaseApplication.getInstance().dbHelper.update(table, cv, "sid=?", whereValue)
                }
            }
    ...
    fun getMaxVersion(): Long {
        if (BageBaseApplication.getInstance().dbHelper == null) {
            return 0
        }
    ...
    private fun serialize(cursor: Cursor): ProhibitWord {
        ...
        word.content = BageCursor.readString(cursor, "content")
        word.version = BageCursor.readLong(cursor, "version")
        word.is_deleted = BageCursor.readInt(cursor, "is_deleted")
        word.id = BageCursor.readInt(cursor, "sid")
        word.created_at = BageCursor.readString(cursor, "created_at")
        return word
    }
}