...
public class AllMembersAdapter extends BaseQuickAdapter<AllGroupMemberEntity, BaseViewHolder> {
    ...
    protected void convert(@NotNull BaseViewHolder baseViewHolder, AllGroupMemberEntity entity) {
        BageChannelMember channelMember = entity.getChannelMember();
        ...
        avatarView.showAvatar(channelMember.memberUID, BageChannelType.PERSONAL, channelMember.memberAvatar);
        if (channelMember.role == BageChannelMemberRole.admin) {
        ...
        } else if (channelMember.role == BageChannelMemberRole.manager) {
            roleTv.setVisibility(View.VISIBLE);
