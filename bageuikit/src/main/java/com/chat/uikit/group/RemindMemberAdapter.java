...
public class RemindMemberAdapter extends BaseQuickAdapter<GroupMemberEntity, BaseViewHolder> {
    ...
    protected void convert(@NonNull BaseViewHolder baseViewHolder, GroupMemberEntity groupMemberEntity) {
        ...
        if (groupMemberEntity.member == null) {
            ...
            avatarView.showAvatar(groupMemberEntity.member.memberUID, BageChannelType.PERSONAL, groupMemberEntity.member.memberAvatarCacheKey);
        }
    }
    ...
    private void getChannelMember(boolean isNormal) {
        int size = 100;
        BageIM.getInstance().getChannelMembersManager().getWithPageOrSearch(channelID, channelType, searchKey, page, size, (list, b) -> resort(list, isNormal));
    }

    private void resort(List<BageChannelMember> list, boolean isNormal) {
        ...
        if (page == 1 && isNormal) {
            ...
            if (channelMember != null && channelMember.role != BageChannelMemberRole.normal) {
