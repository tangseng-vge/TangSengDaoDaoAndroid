 ...
 * 群成员适配器
 */
public class GroupMemberAdapter extends BaseQuickAdapter<BageChannelMember, BaseViewHolder> {
    public GroupMemberAdapter(@Nullable List<BageChannelMember> data) {
        super(R.layout.item_group_member_layout, data);
    }

    @Override
    protected void convert(@NonNull BaseViewHolder helper, BageChannelMember item) {
        ...
        if (item.memberUID.equalsIgnoreCase("-1")) {
            ...
            avatarView.showAvatar(item.memberUID, BageChannelType.PERSONAL, item.memberAvatarCacheKey);
            helper.setGone(R.id.handlerIv, true);
            helper.setGone(R.id.userLayout, false);
            if (item.role == BageChannelMemberRole.admin) {
            ...
            } else if (item.role == BageChannelMemberRole.manager) {
