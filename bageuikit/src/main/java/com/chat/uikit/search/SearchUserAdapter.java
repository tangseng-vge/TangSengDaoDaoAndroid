    ...
    protected void convert(@NonNull BaseViewHolder helper, SearchUserEntity item) {
        switch (item.getItemType()) {
            ...
            case 0:
                ...
                avatarView.showAvatar(item.data.uid, BageChannelType.PERSONAL);
                helper.setText(R.id.nameTv, item.data.name);
                helper.setGone(R.id.applyBtn, !item.showApply || !TextUtils.isEmpty(item.data.uid) && item.data.uid.equals(BageConfig.getInstance().getUid()));
