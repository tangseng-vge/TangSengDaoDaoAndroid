 ...
 * 语音管理类
 */
public class BageVoiceViewManager {

    private BageVoiceViewManager() {

    }

    private static class VoiceViewManagerBinder {
        final static BageVoiceViewManager manager = new BageVoiceViewManager();
    }

    public static BageVoiceViewManager getInstance() {
    ...
    public View getVoiceView(IConversationContext iConversationContext) {
        ...
        recordAudioView.setRecordAudioListener(new RecordAudioView.IRecordAudioListener() {
            ...
            public String onRecordStart() {
                ...
                audioFileName = BageUIKitApplication.getInstance().getContext().getExternalCacheDir() + File.separator + createAudioName();
...
//                    int duration = AudioPlaybackManager.getDuration(audioFileName);
                    ...
                    BageVoiceContent audioMsgModel = new BageVoiceContent(audioFileName, time);
                    byte[] dbs = AudioRecordManager.getInstance().getDbs();
                    audioMsgModel.waveform = BageCommonUtils.getInstance().base64Encode(dbs);
