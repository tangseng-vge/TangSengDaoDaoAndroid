...
class ErrorLogsActivity : BageBaseActivity<ActCommonListLayoutBinding>() {
    ...
    fun getData(): ArrayList<LogEntity> {
        val path = BageFileUtils.getInstance().getNormalFileSavePath("bageCrash")
        ...
        for (i in tempList!!.indices) {
            if (tempList[i].isFile) {
                val size = BageFileUtils.getInstance().getFileSize(File(tempList[i].toString()))
        ...
        if (BageReader.isNotEmpty(fileList))
    ...
    class FileAdapter : BaseQuickAdapter<LogEntity, BaseViewHolder>(R.layout.item_logger_layout) {
        override fun convert(holder: BaseViewHolder, item: LogEntity) {
            ...
            holder.setText(R.id.timeTv, BageTimeUtils.getInstance().time2DateStr1(item.time))
