 ...
 * 会话
 */
public class ChatFragment extends BageBaseFragment<FragChatConversationLayoutBinding> {
    ...
    protected void initView() {
        bageVBinding.textSwitcher.setTag(-1);
        bageVBinding.textSwitcher.setFactory(() -> {
        ...
        bageVBinding.textSwitcher.setText(getString(R.string.app_name));
        //去除刷新条目闪动动画
        ...
        initAdapter(bageVBinding.recyclerView, chatConversationAdapter);
        ...
        Theme.setPressedBackground(bageVBinding.deviceIv);
        Theme.setPressedBackground(bageVBinding.rightIv);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ViewCompat.setTransitionName(bageVBinding.ivSearch, "searchView");
        }
    }
    ...
    protected void initListener() {
        bageVBinding.rightIv.setOnClickListener(view -> {
            ...
            BageDialogUtils.getInstance().showScreenPopupBelow(view, list, 0);
        });

        bageVBinding.deviceIv.setOnClickListener(v -> EndpointManager.getInstance().invoke("show_pc_login_view", getActivity()));
        bageVBinding.rlSearch.setOnClickListener(v -> openGlobalSearch());
        chatConversationAdapter.setRecyclerView(bageVBinding.recyclerView);
        bageVBinding.recyclerView.addOnItemTouchListener(new RecyclerView.SimpleOnItemTouchListener() {
            @Override
        ...
        chatConversationAdapter.setOnItemChildClickListener((adapter, view, position) -> SingleClickUtil.determineTriggerSingleClick(view, v -> {
            ...
            RecyclerView.ViewHolder holder = bageVBinding.recyclerView.findViewHolderForAdapterPosition(position);
            ...
            if (uiConversationMsg != null && uiConversationMsg.uiConversationMsg != null) {
                if (view.getId() == R.id.contentLayout) {
                    if (uiConversationMsg.uiConversationMsg.channelType == BageChannelType.COMMUNITY) {
                        EndpointManager.getInstance().invoke("show_community", uiConversationMsg.uiConversationMsg.channelID);
                    } else
                        BageIMUtils.getInstance().startChatActivity(new ChatViewMenu(getActivity(), uiConversationMsg.uiConversationMsg.channelID, uiConversationMsg.uiConversationMsg.channelType, 0, false));
                }
            }
        }));
        chatConversationAdapter.addListener((menu, item) -> {
            if (menu == ChatConversationAdapter.ItemMenu.delete) {
                BageDialogUtils.getInstance().showDialog(getActivity(), getString(R.string.delete_chat), getString(R.string.delete_conver_msg_tips), true, "", getString(R.string.base_delete), 0, ContextCompat.getColor(requireActivity(), R.color.red), index -> {
                    if (index == 1) {
                        List<BageReminder> list = BageIM.getInstance().getReminderManager().getReminders(item.channelID, item.channelType);
                        if (BageReader.isNotEmpty(list)) {
                            ...
                            if (BageReader.isNotEmpty(reminderIds))
                        ...
                        boolean result = BageIM.getInstance().getConversationManager().deleteWitchChannel(item.channelID, item.channelType);
                        if (result) {
                            if (item.getBageChannel() != null && item.getBageChannel().top == 1) {
                                updateTop(item.channelID, item.channelType, 0);
                            }
                            BageIM.getInstance().getMsgManager().clearWithChannel(item.channelID, item.channelType);
                        }
                    }
                });
                ...
                //免打扰
                if (item.channelType == BageChannelType.GROUP) {
        ...
        //频道刷新监听
        BageIM.getInstance().getChannelManager().addOnRefreshChannelInfo("chat_fragment_refresh_channel", (channel, isEnd) -> {
            if (channel != null) {
                for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
                    if (!TextUtils.isEmpty(chatConversationAdapter.getData().get(i).uiConversationMsg.channelID) && !TextUtils.isEmpty(channel.channelID) && chatConversationAdapter.getData().get(i).uiConversationMsg.channelID.equals(channel.channelID) && chatConversationAdapter.getData().get(i).uiConversationMsg.channelType == channel.channelType) {
                        ...
                        chatConversationAdapter.getData().get(i).uiConversationMsg.setBageChannel(channel);
        ...
        //监听移除最近会话
        BageIM.getInstance().getConversationManager().addOnDeleteMsgListener("chat_fragment", (s, b) -> {
        ...
        BageIM.getInstance().getCMDManager().addCmdListener("chat_fragment_cmd", bageCmd -> {
            ...
            switch (bageCmd.cmdKey) {
                case BageCMDKeys.bage_typing -> {
                    ...
                    for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
                        if (chatConversationAdapter.getData().get(i).uiConversationMsg.channelID.equals(channelID) && chatConversationAdapter.getData().get(i).uiConversationMsg.channelType == channelType) {
                            ...
                            chatConversationAdapter.getData().get(i).typingStartTime = BageTimeUtils.getInstance().getCurrentSeconds();
                            notifyRecycler(i, chatConversationAdapter.getData().get(i));
                            if (disposable == null) {
                                startTimer();
                            }
                ...
                case BageCMDKeys.bage_onlineStatus -> {
                    if (bageCmd.paramJsonObject != null) {
                        int device_flag = bageCmd.paramJsonObject.optInt("device_flag");
                        int online = bageCmd.paramJsonObject.optInt("online");
                        String uid = bageCmd.paramJsonObject.optString("uid");
                        if (uid.equals(BageConfig.getInstance().getUid()) && device_flag == 1) {
                            bageVBinding.deviceIv.setVisibility(online == 1 ? View.VISIBLE : View.GONE);
                            BageSharedPreferencesUtil.getInstance().putInt(BageConfig.getInstance().getUid() + "_pc_online", online);
                        }
                    }
                }
                case "sync_channel_state" -> {
                    String fromUID = bageCmd.paramJsonObject.optString("from_uid");
                    String channelId = bageCmd.paramJsonObject.optString("channel_id");
                    int channelType = bageCmd.paramJsonObject.optInt("channel_type");
                    if (channelId.equals(BageConfig.getInstance().getUid())) {
                        channelId = fromUID;
                    }
                    String finalChannelId = channelId;
                    BageCommonModel.getInstance().getChannelState(channelId, (byte) channelType, channelState -> {
                        if (channelState != null) {
                            int isCalling = 0;
                            if (BageReader.isNotEmpty(channelState.call_info.getCalling_participants())) {
                                isCalling = 1;
                            }
                            for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
        ...
        // 监听刷新消息
        BageIM.getInstance().getMsgManager().addOnRefreshMsgListener("chat_fragment", (msg, left) -> {
            if (msg == null) return;
            for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
                if (chatConversationAdapter.getData().get(i).uiConversationMsg.channelID.equals(msg.channelID) && chatConversationAdapter.getData().get(i).uiConversationMsg.channelType == msg.channelType && chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg() != null && (chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().clientSeq == msg.clientSeq || chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().clientMsgNO.equals(msg.clientMsgNO))) {
                    if (chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().status != msg.status || chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().remoteExtra.readedCount != msg.remoteExtra.readedCount) {
                        chatConversationAdapter.getData().get(i).isRefreshStatus = true;
                    }
                    if (chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().remoteExtra.revoke != msg.remoteExtra.revoke) {
                        chatConversationAdapter.getData().get(i).isResetContent = true;
                    }
                    chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().status = msg.status;
                    if (chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().remoteExtra.editedAt != msg.remoteExtra.editedAt) {
                        chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().remoteExtra.editedAt = msg.remoteExtra.editedAt;
                        chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().remoteExtra.contentEdit = msg.remoteExtra.contentEdit;
                        BageIMUtils.getInstance().resetMsgProhibitWord(chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg());
                    }
                    chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().remoteExtra.revoker = msg.remoteExtra.revoker;
                    chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().remoteExtra.revoke = msg.remoteExtra.revoke;
                    chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().remoteExtra.unreadCount = msg.remoteExtra.unreadCount;
                    chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().remoteExtra.readedCount = msg.remoteExtra.readedCount;
                    chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().messageID = msg.messageID;
                    refreshIds.add(i);
                    break;
                }
            }
            if (left && BageReader.isNotEmpty(refreshIds)) {
...
//                    chatConversationAdapter.notifyItemChanged(refreshIds.get(i), chatConversationAdapter.getData().get(refreshIds.get(i)));
        ...
        BageIM.getInstance().getMsgManager().addOnClearMsgListener("chat_fragment", (channelID, channelType, fromUID) -> {
            if (TextUtils.isEmpty(fromUID))
                for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
                    if (chatConversationAdapter.getData().get(i).uiConversationMsg.channelID.equals(channelID) && chatConversationAdapter.getData().get(i).uiConversationMsg.channelType == channelType) {
                        chatConversationAdapter.getData().get(i).uiConversationMsg.setBageMsg(null);
                        chatConversationAdapter.getData().get(i).isResetContent = true;
                        notifyRecycler(i, chatConversationAdapter.getData().get(i));
//                        chatConversationAdapter.notifyItemChanged(i, chatConversationAdapter.getData().get(i));
        ...
        BageIM.getInstance().getReminderManager().addOnNewReminderListener("chat_fragment", list -> {
            if (BageReader.isEmpty(list) || BageReader.isEmpty(chatConversationAdapter.getData()))
                return;
            for (BageReminder reader : list) {
                for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
                    if (reader.done == 0
                            && !TextUtils.isEmpty(reader.messageID)
                            && chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg() != null
                            && !TextUtils.isEmpty(chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().messageID)
                            && chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg() != null
                            && reader.messageID.equals(chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().messageID)) {
        ...
        // 监听刷新最近列表
        BageIM.getInstance().getConversationManager().addOnRefreshMsgListListener("chat_fragment", list -> {
            ...
            if (chatConversationAdapter.getData().isEmpty()) {
                ...
                for (BageUIConversationMsg uiConversationMsg : list) {
...
//                            }
                        ...
                        chatConversationAdapter.getData().get(i).uiConversationMsg.setBageMsg(uiConversationMsg.getBageMsg());
...
//        BageIM.getInstance().getConversationManager().addOnRefreshMsgListener("chat_fragment", this::resetData);
        // 监听连接状态
        BageIM.getInstance().getConnectionManager().addOnConnectionStatusListener("chat_fragment", (i, reason) -> {
            if (bageVBinding.textSwitcher.getTag() != null) {
                Object tag = bageVBinding.textSwitcher.getTag();
            ...
            if (i == BageConnectStatus.syncMsg) {
                bageVBinding.textSwitcher.setText(getString(R.string.sync_msg));
            } else if (i == BageConnectStatus.success) {
                bageVBinding.textSwitcher.setText(getString(R.string.app_name));
            } else if (i == BageConnectStatus.connecting) {
                bageVBinding.textSwitcher.setText(getString(R.string.connecting));
            } else if (i == BageConnectStatus.noNetwork) {
                bageVBinding.textSwitcher.setText(getString(R.string.network_error_tips));
            } else if (i == BageConnectStatus.kicked) {
                int from = 0;
                if (reason.equals(BageConnectReason.ReasonConnectKick)) {
                    from = 1;
                }
                BageUIKitApplication.getInstance().exitLogin(from);
            }
            bageVBinding.textSwitcher.setTag(i);
            if (i != BageConnectStatus.success && i != BageConnectStatus.syncMsg) {
                startConnectTimer();
            } else {
                EndpointManager.getInstance().invoke("bage_close_disconnect_screen", null);
                stopConnectTimer();
            }
        });
        EndpointManager.getInstance().setMethod("", EndpointCategory.bageExitChat, object -> {
            if (object != null) {
        ...
        EndpointManager.getInstance().setMethod("chat_fragment", EndpointCategory.bageRefreshChatConversation, object -> {
        ...
        EndpointManager.getInstance().setMethod("chat_cover", EndpointCategory.refreshProhibitWord, object -> {
            if (BageReader.isEmpty(chatConversationAdapter.getData())) {
                return 1;
            }
            for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
                if (chatConversationAdapter.getData().get(i).uiConversationMsg != null && chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg() != null && chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg().type == BageContentType.Bage_TEXT) {
                    BageIMUtils.getInstance().resetMsgProhibitWord(chatConversationAdapter.getData().get(i).uiConversationMsg.getBageMsg());
                    chatConversationAdapter.notifyItemChanged(i);
                }
            }
        ...
        EndpointManager.getInstance().setMethod("refresh_conversation_calling", object -> {
            if (BageReader.isNotEmpty(MsgModel.getInstance().channelStatus)) {
                for (BageChannelState state : MsgModel.getInstance().channelStatus) {
                    for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
                        if (chatConversationAdapter.getData().get(i).uiConversationMsg != null
                                && !TextUtils.isEmpty(chatConversationAdapter.getData().get(i).uiConversationMsg.channelID)
    ...
    private void getChatMsg() {
        BageIM.getInstance().getConversationManager().getAll(new IAllConversations() {
            @Override
            public void onResult(List<BageUIConversationMsg> list) {
...
//        List<BageUIConversationMsg> tempList = BageIM.getInstance().getConversationManager().getAll();
...
//        return list;
    ...
    private void setAllCount() {
        int allCount = 0;
        for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
            if (chatConversationAdapter.getData().get(i).uiConversationMsg.getBageChannel() != null && chatConversationAdapter.getData().get(i).uiConversationMsg.getBageChannel().mute == 0)
    ...
    private void resetChildData(BageUIConversationMsg uiConversationMsg, boolean isEnd) {
        if (BageReader.isNotEmpty(chatConversationAdapter.getData())) {
            boolean isAdd = true;
            for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
                boolean isBreak = false;
                if (BageReader.isNotEmpty(chatConversationAdapter.getData().get(i).childList)) {
    ...
    private void resetData(BageUIConversationMsg uiConversationMsg, boolean isEnd) {
        if (uiConversationMsg == null) {
            return;
        }
        // || (uiConversationMsg.getBageChannel() != null && uiConversationMsg.getBageChannel().follow == 0 && uiConversationMsg.channelType == BageChannelType.PERSONAL)
    ...
    private void sortMsg(List<ChatConversationMsg> list) {
        ...
        for (int i = 0, size = list.size(); i < size; i++) {
            if (list.get(i).uiConversationMsg.getBageChannel() != null && list.get(i).uiConversationMsg.getBageChannel().top == 1) {
            ...
            public void onNext(@NonNull Long value) {
                ...
                for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
                    if (chatConversationAdapter.getData().get(i).typingStartTime > 0) {
                        ...
                        if (BageTimeUtils.getInstance().getCurrentSeconds() - typingStartTime >= 8) {
                            chatConversationAdapter.getData().get(i).isResetTyping = true;
                            chatConversationAdapter.getData().get(i).typingStartTime = 0;
                            chatConversationAdapter.getData().get(i).typingUserName = "";
                            chatConversationAdapter.getData().get(i).isResetContent = true;
...
//                                    chatConversationAdapter.notifyItemChanged(i, chatConversationAdapter.getData().get(i));
    ...
    public void onDestroy() {
        ...
        BageIM.getInstance().getConversationManager().removeOnRefreshMsgListListener("chat_fragment");
        BageIM.getInstance().getConversationManager().removeOnRefreshMsgListener("chat_fragment");
        BageIM.getInstance().getConversationManager().removeOnDeleteMsgListener("chat_fragment");
        BageIM.getInstance().getCMDManager().removeCmdListener("chat_fragment_cmd");
        BageIM.getInstance().getMsgManager().removeRefreshMsgListener("chat_fragment");
        BageIM.getInstance().getConnectionManager().removeOnConnectionStatusListener("chat_fragment");
        BageIM.getInstance().getMsgManager().removeSendMsgAckListener("chat_fragment");
        BageIM.getInstance().getReminderManager().removeNewReminderListener("chat_fragment");
    }
    ...
    private void openGlobalSearch() {
        dismissOpenedSwipe();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            @SuppressWarnings("unchecked") ActivityOptionsCompat activityOptions = ActivityOptionsCompat.makeSceneTransitionAnimation(requireActivity(), new Pair<>(bageVBinding.ivSearch, "searchView"));
    ...
    public void onResume() {
        super.onResume();
        dismissOpenedSwipe();
        int pcOnline = BageSharedPreferencesUtil.getInstance().getInt(BageConfig.getInstance().getUid() + "_pc_online");
        bageVBinding.deviceIv.setVisibility(pcOnline == 1 ? View.VISIBLE : View.GONE);
...
//        if (muteForApp == 1) {
//            pcLoginTv.setText(String.format("%s %s", appLoginType, getString(R.string.bage_kit_phone_notice_close)));
//        } else pcLoginTv.setText(appLoginType);
    ...
    private int getTopChatCount() {
        int count = 0;
        for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
            if (chatConversationAdapter.getData().get(i).uiConversationMsg.getBageChannel() != null && chatConversationAdapter.getData().get(i).uiConversationMsg.getBageChannel().top == 1)
                count++;
        }
        return count;
    }

    private int getInsertIndex(BageUIConversationMsg msg) {
        if (msg.getBageChannel() != null && msg.getBageChannel().top == 1) return 0;
        return getTopChatCount();
    }

    private void notifyRecycler(int index, ChatConversationMsg msg) {
        if (bageVBinding.recyclerView.getScrollState() == RecyclerView.SCROLL_STATE_IDLE || (!bageVBinding.recyclerView.isComputingLayout())) {
    ...
    private void groupMsg(List<ChatConversationMsg> list) {
        ...
        if (!msgMap.isEmpty()) {
            for (String key : msgMap.keySet()) {
                ...
                BageUIConversationMsg lastMsg = new BageUIConversationMsg();
...
//                }
                ...
                if (BageReader.isNotEmpty(msgList)) {
                    ...
                    int unreadCount = 0;
                    List<BageReminder> reminderList = new ArrayList<>();
                    for (int i = 0, size = msgList.size(); i < size; i++) {
                        BageUIConversationMsg msg = msgList.get(i).uiConversationMsg;
                        ...
                        List<BageReminder> tempReminders = msg.getReminderList();
                        if (BageReader.isNotEmpty(tempReminders)) {
    ...
    private void scrollToUnreadChannel() {
        ...
        LinearLayoutManager linearLayoutManager = (LinearLayoutManager) bageVBinding.recyclerView.getLayoutManager();
        for (int i = 0, size = chatConversationAdapter.getData().size(); i < size; i++) {
            if (chatConversationAdapter.getData().get(i).getUnReadCount() > 0 && chatConversationAdapter.getData().get(i).uiConversationMsg.getBageChannel() != null && chatConversationAdapter.getData().get(i).uiConversationMsg.getBageChannel().mute == 0) {
                if (firstTime == 0) {
