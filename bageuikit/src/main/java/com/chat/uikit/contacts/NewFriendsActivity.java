 ...
 * 新朋友
 */
public class NewFriendsActivity extends BageBaseActivity<ActCommonListLayoutBinding> {
    ...
    protected void initView() {
        ...
        BageSharedPreferencesUtil.getInstance().putInt(BageConfig.getInstance().getUid() + "_new_friend_count", 0);
    }
                ...
                if (view1.getId() == R.id.agreeBtn) {
                    FriendModel.getInstance().agreeFriendApply(newFriendEntity.token, (code, msg) -> {
                        if (code == HttpResponseCode.success) {
                            ...
                            BageContactsDB.getInstance().updateFriendStatus(newFriendEntity.apply_uid, 1);
                        }
                    });
                }
            }
    ...
    protected void initData() {
        ...
        List<BageChannel> friends = BageIM.getInstance().getChannelManager().getChannels(uidList);
        if (BageReader.isNotEmpty(friends)) {
            for (BageChannel channel : friends) {
