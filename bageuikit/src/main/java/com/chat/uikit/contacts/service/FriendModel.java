 ...
 * 好友管理
 */
public class FriendModel extends BageBaseModel {
    ...
    public void syncFriends(final ICommonListener iCommonListener) {
        String key = String.format("%s_friend_sync_version", BageConfig.getInstance().getUid());
        long version = BageSharedPreferencesUtil.getInstance().getLong(key);
            ...
            public void onSuccess(List<UserInfo> list) {
                if (BageReader.isNotEmpty(list)) {
                    ...
                    for (int i = 0, size = list.size(); i < size; i++) {
                        ...
                        channel.channelType = BageChannelType.PERSONAL;//指定为单聊信息
                        ...
                        hashMap.put(BageChannelExtras.revokeRemind, list.get(i).revoke_remind);
                        hashMap.put(BageChannelExtras.screenshot, list.get(i).screenshot);
                        hashMap.put(BageChannelExtras.sourceDesc, list.get(i).source_desc);
                        hashMap.put(BageChannelExtras.chatPwdOn, list.get(i).chat_pwd_on);
                        hashMap.put(BageChannelExtras.vercode, list.get(i).vercode);
                    ...
                    BageSharedPreferencesUtil.getInstance().putLong(key, tempVersion);
                    //将好友信息设置到sdk
                    dispatchQueuePool.execute(() -> {
                        BageIM.getInstance().getChannelManager().saveOrUpdateChannels(channels);
                    });
    ...
    public void saveNewFriendsMsg(String contentJson) {
        ...
        if (newFriendEntity1 != null && !TextUtils.isEmpty(newFriendEntity1.apply_uid)) {
            ...
            newFriendEntity1.created_at = BageTimeUtils.getInstance().getNowDate1();
            ApplyDB.getInstance().update(newFriendEntity1);
        } else {
            newFriendEntity.created_at = BageTimeUtils.getInstance().getNowDate1();
            ApplyDB.getInstance().insert(newFriendEntity);
        }
        int new_friend_count = BageSharedPreferencesUtil.getInstance().getInt(BageConfig.getInstance().getUid() + "_new_friend_count");
        new_friend_count++;
        BageSharedPreferencesUtil.getInstance().putInt(BageConfig.getInstance().getUid() + "_new_friend_count", new_friend_count);
