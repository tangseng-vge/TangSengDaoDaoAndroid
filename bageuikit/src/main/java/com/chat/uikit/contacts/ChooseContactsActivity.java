 ...
 * 选择联系人
 */
public class ChooseContactsActivity extends BageBaseActivity<ActChooseContactsLayoutBinding> implements OnQuickSideBarTouchListener {
    ...
    //默认选中的用户
    private List<BageChannel> defaultSelected;
    private int type;//0：创建群聊1：群聊加人2：选择用户
    private String groupId;
    private List<BageMessageContent> msgContentList;
    ...
    protected void initView() {
        ...
        initAdapter(bageVBinding.recyclerView, contactsAdapter);
...
//        List<FriendUIEntity> list = contactsAdapter.getData();
        ...
        if (type == 0 || type == 1) {
            ...
            if (type == 0) {
                if (ids.size() == 1) {
                    BageIMUtils.getInstance().startChatActivity(new ChatViewMenu(this, ids.get(0), BageChannelType.PERSONAL, 0, true, msgContentList));
                    finish();
                } else {
                    GroupModel.getInstance().createGroup(name.toString(), ids, names, (code, msg, groupEntity) -> {
                        ...
                        if (code == HttpResponseCode.success && groupEntity != null) {
                            BageIMUtils.getInstance().startChatActivity(new ChatViewMenu(this, groupEntity.group_no, BageChannelType.GROUP, 0, true, msgContentList));
                            finish();
                        } else {
                ...
                if (channel != null) {
                    if (channel.invite == 1) {
                        //邀请确认
                        BageChannelMember member = BageIM.getInstance().getChannelMembersManager().getMember(groupId, BageChannelType.GROUP, BageConfig.getInstance().getUid());
                        if (member == null || member.isDeleted == 1) return;
                        addType = member.role == BageChannelMemberRole.normal ? 2 : 1;
                    }
                }
                ...
                if (addType == 1) {
                    ...
                    GroupModel.getInstance().inviteGroupMembers(groupId, ids, (code, msg) -> {
                        if (code == HttpResponseCode.success) {
                            BageToastUtils.getInstance().showToast(getString(R.string.invitation_sent));
                            setResult(RESULT_OK);
                            finish();
                        } else {
                            hideTitleRightLoading();
            ...
            List<BageChannel> selectedList = new ArrayList<>();
            for (int i = 0, size = list.size(); i < size; i++) {
                if (list.get(i).check) {
                    selectedList.add(list.get(i).channel);
                }
            }
            if (BageReader.isNotEmpty(selectedList) && isShowSaveLabelDialog) {
                BageDialogUtils.getInstance().showDialog(this, getString(R.string.message_tips), getString(R.string.save_label_tips), true, getString(R.string.cancel), getString(R.string.save_label_sure), 0, Theme.colorAccount, index -> {
                    if (index == 1) {
                        EndpointManager.getInstance().invoke("save_label", new SaveLabelMenu(ChooseContactsActivity.this, selectedList));
                        finish();
                    } else {
                        BageUIKitApplication.getInstance().setChooseContactsBack(selectedList);
                        finish();
                    }
                });
            } else {
            ...
            if (friendEntity != null) {
                if (singleChoose && chooseBack) {
                    ...
                    if (!TextUtils.isEmpty(groupId) && UserUtils.getInstance().checkGroupBlacklist(groupId, friendEntity.channel.channelID)) {
                        BageToastUtils.getInstance().showToastFail(BageUIKitApplication.getInstance().getContext().getString(R.string.no_group_blacklist_noselect));
                        return;
                    }
    ...
    protected void initData() {
        super.initData();
        BageAPPConfig bageappConfig = BageConfig.getInstance().getAppConfig();
        int inviteSystemAccountJoinGroupOn = 0;
        if (bageappConfig != null) {
            inviteSystemAccountJoinGroupOn = bageappConfig.invite_system_account_join_group_on;
        }
        List<BageChannel> tempList = BageIM.getInstance().getChannelManager().getWithFollowAndStatus(BageChannelType.PERSONAL, 1, 1);
        ...
        for (int i = 0, size = tempList.size(); i < size; i++) {
            if (!TextUtils.isEmpty(unVisibleUIDs) && unVisibleUIDs.contains(tempList.get(i).channelID))
                continue;
            if (inviteSystemAccountJoinGroupOn == 0 && tempList.get(i).channelID.equals(BageSystemAccount.system_file_helper)) {
                continue;
            }
            ...
            if (type == 2 && BageReader.isNotEmpty(defaultSelected)) {
        ...
        if (BageReader.isNotEmpty(list)) {
            for (int i = 0, size = list.size(); i < size; i++) {
                if (list.get(i).pying.startsWith(letter)) {
                    bageVBinding.recyclerView.smoothScrollToPosition(i);
                    break;
                }
            }
        }
