...
class FriendAdapter :
    ...
    override fun convert(holder: BaseViewHolder, item: FriendUIEntity) {
        ...
        if (!TextUtils.isEmpty(item.channel.category)) {
            if (item.channel.category == BageSystemAccount.accountCategorySystem) {
            ...
            if (item.channel.category == BageSystemAccount.accountCategoryCustomerService) {
                linearLayout.addView(
                    Theme.getChannelCategoryTV(
                        context,
            ...
            if (item.channel.category == BageSystemAccount.accountCategoryVisitor) {
                linearLayout.addView(
                    Theme.getChannelCategoryTV(
                        context,
        ...
        if (item.channel.online == 1) {
            ...
            if (item.channel.lastOffline == 0L) {
                ...
                val lastSeenTime =
                    BageTimeUtils.getInstance().getOnlineTime(item.channel.lastOffline)
                if (TextUtils.isEmpty(lastSeenTime)) {
                    holder.setGone(R.id.offlineTv, false)
                    val time = BageTimeUtils.getInstance()
