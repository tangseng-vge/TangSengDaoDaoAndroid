...
public class MyReportsActivity extends BageBaseActivity<ActMyReportsBinding> {
    ...
    protected void initListener() {
        bageVBinding.refreshLayout.setOnRefreshListener(refreshLayout -> {
            pageIndex = 1;
            loadReports();
        });
        bageVBinding.refreshLayout.setOnLoadMoreListener(refreshLayout -> loadReports());
    }
    ...
    private void loadReports() {
        ...
        MomentsModel.getInstance().myReports(lang, requestedPage, PAGE_SIZE, (code, msg, result) -> {
            ...
            if (code != HttpResponseCode.success || result == null) {
                if (!TextUtils.isEmpty(msg)) BageToastUtils.getInstance().showToastFail(msg);
                updateEmptyView();
                return;
            }
            ...
            bageVBinding.refreshLayout.setEnableLoadMore(more);
    ...
    private void updateEmptyView() {
        bageVBinding.emptyTv.setVisibility(records.isEmpty() ? View.VISIBLE : View.GONE);
    }
}
