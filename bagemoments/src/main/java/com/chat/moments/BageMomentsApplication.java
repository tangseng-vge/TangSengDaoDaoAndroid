 ...
 * 动态
 */
public class BageMomentsApplication {
    private BageMomentsApplication() {
    }

    private static class MomentsApplicationBinder {
        final static BageMomentsApplication app = new BageMomentsApplication();
    }

    public static BageMomentsApplication getInstance() {
    ...
    public void init(final Context context) {
        //初始化数据库信息
        EndpointManager.getInstance().setMethod("moments", EndpointCategory.bageDBMenus, object -> new DBMenu("moments_sql"));
        AppModule appModule = BageBaseApplication.getInstance().getAppModuleWithSid("moment");
        ...
        EndpointManager.getInstance().setMethod(EndpointCategory.mailList + "_moments", EndpointCategory.mailList, 200, object -> {
            ...
            contactsMenu.badgeNum = BageSharedPreferencesUtil.getInstance().getInt(BageConfig.getInstance().getUid() + "_moments_msg_count");
            contactsMenu.uid = BageSharedPreferencesUtil.getInstance().getSP(BageConfig.getInstance().getUid() + "_moments_msg_uid");
            return contactsMenu;
        });

        //监听登录成功后将朋友圈背景清空重新下载
        EndpointManager.getInstance().setMethod("", EndpointCategory.loginMenus, object -> {
            BageSharedPreferencesUtil.getInstance().putSP(String.format("moment_bg_url_%s", BageConfig.getInstance().getUid()), "");
            return null;
        });
        //获取要显示在通讯录的红点数量
        EndpointManager.getInstance().setMethod("moments", EndpointCategory.bageGetMailListRedDot, object -> {
            int count = BageSharedPreferencesUtil.getInstance().getInt(BageConfig.getInstance().getUid() + "_moments_msg_count");
            String uid = BageSharedPreferencesUtil.getInstance().getSP(BageConfig.getInstance().getUid() + "_moments_msg_uid");
    ...
    private void initListener() {
        BageIM.getInstance().getCMDManager().addCmdListener("moment_cmd", cmd -> {
            if (cmd != null && !TextUtils.isEmpty(cmd.cmdKey) && cmd.paramJsonObject != null) {
                if (cmd.cmdKey.equals(BageCMDKeys.bage_momentMsg)) {
                    if (cmd.paramJsonObject.has("action")) {
                        ...
                        if (action.equals("publish")) {
                            ...
                            BageSharedPreferencesUtil.getInstance().putSP(BageConfig.getInstance().getUid() + "_moments_msg_uid", uid);
        ...
        EndpointManager.getInstance().setMethod("moment", EndpointCategory.bageUserDetailView, 10, object -> {
    ...
    private void saveMomentMsg(JSONObject jsonObject) {
        ...
        if (mMomentsDBMsg.is_deleted == 0) {
            int count = BageSharedPreferencesUtil.getInstance().getInt(BageConfig.getInstance().getUid() + "_moments_msg_count");
            count = count + 1;
            BageSharedPreferencesUtil.getInstance().putInt(BageConfig.getInstance().getUid() + "_moments_msg_count", count);
            //操作者的用户ID
            BageSharedPreferencesUtil.getInstance().putSP(BageConfig.getInstance().getUid() + "_moments_msg_action_uid", mMomentsDBMsg.uid);
            //刷新通讯录红点
            EndpointManager.getInstance().invokes(EndpointCategory.bageRefreshMailList, null);
        }
    ...
    private View getMomentInUserDetailView(WeakReference<Context> context, String uid, ViewGroup parentView) {
        ...
        BageChannel userChannel = BageIM.getInstance().getChannelManager().getChannel(uid, BageChannelType.PERSONAL);
    ...
    private void addMomentImg(List<Moments> list, Context context, String uid, LinearLayout momentImgLayout) {
        ...
        for (int i = 0; i < max; i++) {
            ...
            String url;
            if (BageReader.isNotEmpty(list.get(i).imgs)) {
                url = list.get(i).imgs.get(0);
            } else {
                url = list.get(i).video_cover_path;
                isAddVideo = true;
            }
            GlideUtils.getInstance().showImg(context, BageApiConfig.getShowUrl(url), view);
