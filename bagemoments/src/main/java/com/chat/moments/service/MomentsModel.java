 ...
 * 2020-11-25 14:51
 */
public class MomentsModel extends BageBaseModel {
    ...
    public void blockUser(String uid, final ICommonListener listener) {
        request(createService(MomentsService.class).blockUser(uid), new IRequestResultListener<CommonResponse>() {
            ...
            public void onSuccess(CommonResponse result) {
                BageCommonModel.getInstance().getChannel(uid, BageChannelType.PERSONAL, null);
    ...
    private List<Moments> resetData(List<Moments> result) {
        for (int i = 0, size = result.size(); i < size; i++) {
            ...
            if (BageReader.isEmpty(moment.images) && BageReader.isNotEmpty(moment.imgs)) {
                moment.images = new ArrayList<>();
                for (String url : moment.imgs) {
                    moment.images.add(new MomentImage(url, url, url));
                }
            } else if (BageReader.isNotEmpty(moment.images)) {
                for (int imageIndex = 0; imageIndex < moment.images.size(); imageIndex++) {
                    MomentImage image = moment.images.get(imageIndex);
                    String fallback = BageReader.isNotEmpty(moment.imgs) && imageIndex < moment.imgs.size() ? moment.imgs.get(imageIndex) : "";
            ...
            if (TextUtils.isEmpty(result.get(i).video_path)) {
                if (BageReader.isEmpty(result.get(i).imgs)) {
            ...
            long time = BageTimeUtils.getInstance().date2TimeStamp(result.get(i).created_at, "yyyy-MM-dd HH:mm:ss");
            result.get(i).showDate = BageTimeUtils.getInstance().getTimeFormatText(new Date(time * 1000));
            //获取发布者的信息
            BageChannel publisherBageChannel = BageIM.getInstance().getChannelManager().getChannel(result.get(i).publisher, BageChannelType.PERSONAL);
            if (publisherBageChannel != null) {
                if (result.get(i).publisher.equals(BageConfig.getInstance().getUid())) {
                    result.get(i).publisher_name = BageConfig.getInstance().getUserName();
                } else {
                    result.get(i).publisher_name = TextUtils.isEmpty(publisherBageChannel.channelRemark) ? publisherBageChannel.channelName : publisherBageChannel.channelRemark;
                }
                result.get(i).publisherAvatarCacheKey = publisherBageChannel.avatarCacheKey;
            }

            // 获取点赞用户信息
            if (BageReader.isNotEmpty(result.get(i).likes)) {
                for (int j = 0, len = result.get(i).likes.size(); j < len; j++) {
                    BageChannel channel = BageIM.getInstance().getChannelManager().getChannel(result.get(i).likes.get(j).uid, BageChannelType.PERSONAL);
                    if (channel != null) {
            ...
            //设置点赞数据
            result.get(i).praiseSpan = MomentSpanUtils.getInstance().makePraiseSpan(BageMomentsApplication.getInstance().getContext(), result.get(i).likes);
            ...
            if (BageReader.isNotEmpty(result.get(i).remind_uids)) {
                //查询提及到到用户
                StringBuilder stringBuilder = new StringBuilder();
                for (int j = 0, len = result.get(i).remind_uids.size(); j < len; j++) {
                    if (result.get(i).remind_uids.get(j).equals(BageConfig.getInstance().getUid())) {
                        result.get(i).isRemindMe = true;
                        continue;
                    }
                    BageChannel channel = BageIM.getInstance().getChannelManager().getChannel(result.get(i).remind_uids.get(j), BageChannelType.PERSONAL);
                    if (channel != null) {
                ...
                if (result.get(i).isRemindMe) {
                    if (!TextUtils.isEmpty(stringBuilder)) {
                        remindUserNames = String.format("%s %s", BageMomentsApplication.getInstance().getContext().getString(R.string.me), stringBuilder);
                    } else {
                        remindUserNames = BageMomentsApplication.getInstance().getContext().getString(R.string.moment_remind_u);
                    }
                } else {
            ...
            //重置评论
            if (BageReader.isNotEmpty(result.get(i).comments)) {
                for (int j = 0, len = result.get(i).comments.size(); j < len; j++) {
                    BageChannel channel = BageIM.getInstance().getChannelManager().getChannel(result.get(i).comments.get(j).uid, BageChannelType.PERSONAL);
                    if (channel != null) {
                    ...
                    if (!TextUtils.isEmpty(result.get(i).comments.get(j).reply_uid)) {
                        BageChannel replyBageChannel = BageIM.getInstance().getChannelManager().getChannel(result.get(i).comments.get(j).reply_uid, BageChannelType.PERSONAL);
                        if (replyBageChannel != null) {
                            result.get(i).comments.get(j).avatarCacheKey = replyBageChannel.avatarCacheKey;
                            result.get(i).comments.get(j).reply_name = TextUtils.isEmpty(replyBageChannel.channelRemark) ? replyBageChannel.channelName : replyBageChannel.channelRemark;
                        }
                    }
                }
            }
