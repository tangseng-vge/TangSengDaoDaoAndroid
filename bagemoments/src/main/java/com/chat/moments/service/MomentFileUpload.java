 ...
 * 动态文件上传
 */
public class MomentFileUpload extends BageBaseModel {
    ...
    private void getMomentUploadUrl(String localPath, boolean imageVariants, final IGetUploadFileUrl iGetUploadFileUrl) {
        ...
        if (!BageMediaFileUtils.getInstance().isVideoFileType(localPath)) {
            prefix = "png";
        }
        int w = Math.max(options.outWidth, 0);
        int h = Math.max(options.outHeight, 0);
        String path = "/" + BageConfig.getInstance().getUid() + "/" + BageTimeUtils.getInstance().getCurrentMills() + "." + prefix + "@" + w + "x" + h;
        String variants = imageVariants ? "&image_variants=1" : "";
        request(createService(ApiService.class).getUploadFileUrl(BageApiConfig.baseUrl + "file/upload?type=moment&path=" + path + variants), new IRequestResultListener<UploadFileUrl>() {
            @Override
    ...
    public void getMomentCoverUploadUrl(final IGetUploadFileUrl iGetUploadFileUrl) {
        request(createService(ApiService.class).getUploadFileUrl(BageApiConfig.baseUrl + "file/upload?type=momentcover"), new IRequestResultListener<UploadFileUrl>() {
