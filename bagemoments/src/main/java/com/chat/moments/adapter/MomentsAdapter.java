...
public class MomentsAdapter extends BaseMultiItemQuickAdapter<Moments, BaseViewHolder> {
    ...
    protected void convert(@NotNull BaseViewHolder baseViewHolder, Moments moments) {
        if (moments.itemType > 0) {
            ...
            baseViewHolder.setGone(R.id.lldelete, !moments.publisher.equals(BageConfig.getInstance().getUid()));
            ...
            if (!TextUtils.isEmpty(moments.remindUserNames)) {
                if (moments.publisher.equals(BageConfig.getInstance().getUid())) {
            ...
            avatarView.showAvatar(moments.publisher, BageChannelType.PERSONAL, moments.publisherAvatarCacheKey);
            avatarView.setOnClickListener(view1 -> BageMomentsApplication.getInstance().gotoUserDetail(getContext(), moments.publisher));
            baseViewHolder.getView(R.id.publisherNameTv).setOnClickListener(view1 -> BageMomentsApplication.getInstance().gotoUserDetail(getContext(), moments.publisher));
            ...
            list.add(new PopupMenuItem(getContext().getString(R.string.str_dynmaic_copy), R.mipmap.msg_copy, () -> {
                ...
                BageToastUtils.getInstance().showToastNormal(getContext().getString(R.string.copyed));
            }));
            ...
            BageDialogUtils.getInstance().setViewLongClickPopup(baseViewHolder.getView(R.id.contentTv), list);

            //设置评论和点赞布局是否可见
            if (BageReader.isNotEmpty(moments.comments) || BageReader.isNotEmpty(moments.likes)) {
            ...
            if (BageReader.isNotEmpty(moments.comments)) {
            ...
            if (isDetail) {
                ...
                baseViewHolder.setGone(R.id.likeView, BageReader.isEmpty(moments.likes));
            } else {
                //设置是否详情动态
            ...
            switch (moments.itemType) {
                ...
                case MomentsType.one_image: {
                    ...
                    GlideUtils.getInstance().showImg(getContext(), BageApiConfig.getShowUrl(moments.imgs.get(0)), baseViewHolder.getView(R.id.imageView));
                    ...
                    BageDialogUtils.getInstance().setViewLongClickPopup(baseViewHolder.getView(R.id.imageView), imgLongClicklist);
                ...
                case MomentsType.video_text: {
                    ...
                    if (!TextUtils.isEmpty(moments.video_cover_path) && moments.video_cover_path.contains("@") && moments.video_cover_path.contains("x")) {
                        ...
                        GlideUtils.getInstance().showImg(getContext(), BageApiConfig.getShowUrl(moments.video_cover_path), coverIv);
                    }
                    coverIv.setOnClickListener(view -> EndpointManager.getInstance().invoke("play_video", new PlayVideoMenu((Activity) getContext(), coverIv, "", BageApiConfig.getShowUrl(moments.video_path), BageApiConfig.getShowUrl(moments.video_cover_path))));
                }
                break;
            }
        }
    ...
    private void showImg(Moments moments, List<ImageView> imgList, int index, ImageView imageView, String unique_key, String author_uid, String author_name) {
        ...
        for (int i = 0, size = moments.imgs.size(); i < size; i++) {
            ...
            if (BageReader.isNotEmpty(moments.images) && i < moments.images.size()) {
            ...
            tempImgList.add(BageApiConfig.getShowUrl(preview));
            originalImgList.add(BageApiConfig.getShowUrl(original));
        }
        ...
        CustomImageViewerPopup popup = (CustomImageViewerPopup) BageDialogUtils.getInstance().showImagePopup(getContext(), tempImgList, imgList, imageView, index, list1, null, null);
    ...
    private View getDetailReplyView(ViewGroup viewGroup, int index, String momentNo, MomentsReply momentsReply) {
        ...
        avatarView.showAvatar(momentsReply.uid, BageChannelType.PERSONAL, momentsReply.avatarCacheKey);
        ...
        if (TextUtils.equals(momentsReply.uid, BageConfig.getInstance().getUid())) {
            commentMenus.add(new PopupMenuItem(getContext().getString(R.string.str_delete), R.mipmap.msg_delete, () ->
                    BageDialogUtils.getInstance().showDialog(getContext(), getContext().getString(R.string.delete_comment_title), getContext().getString(R.string.str_moment_delete_reply), true, "", getContext().getString(R.string.base_delete), 0, ContextCompat.getColor(getContext(), R.color.red), index1 -> {
        ...
        BageDialogUtils.getInstance().setViewLongClickPopup(contentTv, commentMenus);
        view.setOnClickListener(view1 -> {
            if (momentsReply.uid.equals(BageConfig.getInstance().getUid())) {
                BageDialogUtils.getInstance().showDialog(getContext(), getContext().getString(R.string.delete_comment_title), getContext().getString(R.string.str_moment_delete_reply), true, "", getContext().getString(R.string.base_delete), 0, ContextCompat.getColor(getContext(), R.color.red), index1 -> {
                    if (index1 == 1) {
        ...
        avatarView.setOnClickListener(view1 -> BageMomentsApplication.getInstance().gotoUserDetail(getContext(), momentsReply.uid));
        nameTv.setOnClickListener(view1 -> BageMomentsApplication.getInstance().gotoUserDetail(getContext(), momentsReply.uid));
        return view;
    }
    ...
    public void removeReportedComment(String momentNo, String commentId) {
        ...
        for (int i = 0; i < getData().size(); i++) {
            ...
            if (!TextUtils.equals(moment.moment_no, momentNo) || BageReader.isEmpty(moment.comments)) continue;
    ...
    private void forwardImg(String path) {
        ...
        String[] strings = url.split("@")[1].split("x");
        int[] w_h = ImageUtils.getInstance().getImgWidthAndHeightToDynamic(getContext(), strings[0], strings[1]);
        BageImageContent imageContent = new BageImageContent();
        ...
        EndpointManager.getInstance().invoke(EndpointSID.showChooseChatView, new ChooseChatMenu(new ChatChooseContacts(list1 -> {
            if (BageReader.isNotEmpty(list1)) {
                for (BageChannel channel : list1) {
                    BageIM.getInstance().getMsgManager().send(imageContent, channel);
                }
