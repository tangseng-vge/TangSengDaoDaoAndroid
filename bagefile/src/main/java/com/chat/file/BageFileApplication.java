 ...
 * 文件
 */
public class BageFileApplication {

    private BageFileApplication() {
    }

    private static class FileApplicationBinder {
        final static BageFileApplication FILE = new BageFileApplication();
    }

    public static BageFileApplication getInstance() {
        return FileApplicationBinder.FILE;
    }


    public void init(Context context) {
        AppModule appModule = BageBaseApplication.getInstance().getAppModuleWithSid("file");
        if (!BageBaseApplication.getInstance().appModuleIsInjection(appModule)) {
            return;
        }
        EndpointManager.getInstance().setMethod(EndpointCategory.msgConfig + BageContentType.Bage_FILE, object -> new MsgConfig(true));
        BageIM.getInstance().getMsgManager().registerContentMsg(FileContent.class);
        BageMsgItemViewManager.getInstance().addChatItemViewProvider(BageMsgContentType.Bage_FILE, new FileProvider());
    ...
    private void initFileListener(Context context) {
        EndpointManager.getInstance().setMethod("", EndpointCategory.chatShowBubble, object -> {
            int type = (int) object;
            return type == BageContentType.Bage_FILE;
        });
        EndpointManager.getInstance().setMethod(EndpointCategory.chatFunction + "_chooseFile", EndpointCategory.chatFunction, 94, object -> new ChatFunctionMenu("", R.mipmap.ic_send_file, context.getString(R.string.str_file_file), (iConversationContext) -> {
            BageFileApplication.this.iConversationContext = new WeakReference<>((IConversationContext) object);
            if (iConversationContext.getChatChannelInfo().flame == 1) {
                BageToastUtils.getInstance().showToast(iConversationContext.getChatActivity().getString(R.string.flame_can_not_send_file));
                return;
            }
        ...
        // 搜索聊天文件
        EndpointManager.getInstance().setMethod("search_with_file", EndpointCategory.bageSearchChatContent, 91, object -> new SearchChatContentMenu(context.getString(R.string.str_file_file), (channelID, channelType) -> {
        ...
        // 搜索聊天文件
//        EndpointManager.getInstance().setMethod("search_chat_file", EndpointCategory.bageSearchChatContent, 91, object -> new SearchChatContentMenu(context.getString(R.string.str_file_file), (channelID, channelType) -> {
        ...
        EndpointManager.getInstance().setMethod("forward_file", object -> {
            ...
            EndpointManager.getInstance().invoke(EndpointSID.showChooseChatView, new ChooseChatMenu(new ChatChooseContacts(list -> {
                for (BageChannel channel : list) {
                    ...
                    BageIM.getInstance().getMsgManager().sendWithOptions(fileContent, channel, options);
                }
            }), fileContent));
