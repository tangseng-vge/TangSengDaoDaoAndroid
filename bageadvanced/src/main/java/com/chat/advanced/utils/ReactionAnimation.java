...
public class ReactionAnimation {
    ...
    static int[] getReactionLocation(ChatAdapter chatAdapter, BageMsg bageMsg) {
        int index = 0;
        for (int i = 0, size = Objects.requireNonNull(chatAdapter).getData().size(); i < size; i++) {
            if (chatAdapter.getData().get(i).bageMsg.clientMsgNO.equals(bageMsg.clientMsgNO)) {
                index = i;
                break;
            }
        }
    ...
    public static void show(Context context, ChatAdapter chatAdapter, String json, int[] location, BageMsg msg) {
        boolean isAdded = false;
        if (BageReader.isNotEmpty(msg.reactionList)) {
            for (BageMsgReaction reaction : msg.reactionList) {
                if (reaction.emoji.equals(json) && reaction.uid.equals(BageConfig.getInstance().getUid())) {
                    isAdded = true;
                    break;
                }
            }
