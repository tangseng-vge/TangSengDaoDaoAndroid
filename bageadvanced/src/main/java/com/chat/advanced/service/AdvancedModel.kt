...
class AdvancedModel private constructor() : BageBaseModel() {
    ...
    fun reactionsMsg(channelID: String, channelType: Byte, messageID: String, emoji: String) {
        ...
        request(
                ...
                object : IRequestResultListener<CommonResponse?> {
                    override fun onSuccess(result: CommonResponse?) {}
                    override fun onFail(code: Int, msg: String) {
                        if (TextUtils.isEmpty(msg)) BageToastUtils.getInstance().showToastNormal(msg)
                    }
                })
    }
    ...
    fun wxLogin(code: String) {
        ...
        deviceJson["device_id"] = BageConstants.getDeviceID()
        deviceJson["device_name"] = BageDeviceUtils.getInstance().deviceName
        deviceJson["device_model"] = BageDeviceUtils.getInstance().systemModel
        ...
        requestAndErrorBack(createService(AdvancedService::class.java).wxLogin(jsonObject),
                object : IRequestResultErrorInfoListener<UserInfoEntity?> {
                    override fun onSuccess(userInfo: UserInfoEntity?) {
                        if (userInfo != null) {
                            BageConfig.getInstance().saveUserInfo(userInfo)
                            BageConfig.getInstance().token = userInfo.token
                            if (!TextUtils.isEmpty(userInfo.im_token)) {
                                BageConfig.getInstance().imToken = userInfo.im_token
                            } else BageConfig.getInstance().imToken = userInfo.token
                            BageConfig.getInstance().uid = userInfo.uid
                            BageConfig.getInstance().userName = userInfo.name
    ...
    fun syncReaction(channelID: String?, channelType: Byte) {
        val maxSeq = BageIM.getInstance().msgManager.getMaxReactionSeqWithChannel(channelID, channelType)
        val jsonObject = JSONObject()
        jsonObject["channel_id"] = channelID
        jsonObject["channel_type"] = channelType
        jsonObject["seq"] = maxSeq
        request(createService(AdvancedService::class.java).syncReaction(jsonObject), object : IRequestResultListener<List<BageSyncMsgReaction>> {
            override fun onSuccess(result: List<BageSyncMsgReaction>?) {
                BageIM.getInstance().msgManager.saveMessageReactions(result)
            }
