...
class ChatBgListActivity : BageBaseActivity<ActChatBgListLayoutBinding>() {
    ...
    override fun initView() {
        ...
        if (TextUtils.isEmpty(channelID)) {
            url = BageSharedPreferencesUtil.getInstance().getSPWithUID(ChatBgKeys.chatBgUrl)
        } else {
            val channel =
        ...
        bageVBinding.recyclerView.addItemDecoration(object : RecyclerView.ItemDecoration() {
    ...
    override fun initListener() {
        BageIM.getInstance().channelManager.addOnRefreshChannelInfo(
            ...
            var url = ""
            if (TextUtils.isEmpty(channelID)) {
                url = BageSharedPreferencesUtil.getInstance()
                    .getSPWithUID(ChatBgKeys.chatBgUrl)
            } else {
                val channel =
                    BageIM.getInstance().channelManager.getChannel(
                        channelID,
                        channelType
                    )
                if (channel?.localExtra != null) {
    ...
    override fun finish() {
        super.finish()
        BageIM.getInstance().channelManager.removeRefreshChannelInfo("chat_bg")
    }
}