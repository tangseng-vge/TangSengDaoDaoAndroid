...
public class SearchAllMembersAdapter extends BaseQuickAdapter<GroupMember, BaseViewHolder> {
    ...
    protected void convert(@NotNull BaseViewHolder baseViewHolder, GroupMember member) {
        ...
        avatarView.showAvatar(member.member.memberUID, BageChannelType.PERSONAL, member.member.memberAvatar);
        if (member.member.role == BageChannelMemberRole.admin) {
        ...
        } else if (member.member.role == BageChannelMemberRole.manager) {
