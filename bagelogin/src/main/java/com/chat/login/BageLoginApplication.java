...
import com.chat.login.ui.BageResetLoginPwdActivity;
import com.chat.login.ui.BageWebLoginActivity;
import com.chat.login.ui.BageWebLoginConfirmActivity;
 ...
 * 登录模块
 */
public class BageLoginApplication {
    private BageLoginApplication() {

    }

    private static class LoginApplicationBinder {
        static final BageLoginApplication LOGIN = new BageLoginApplication();
    }

    public static BageLoginApplication getInstance() {
    ...
    public void init(Context context) {
            ...
            if (type.equals("loginConfirm")) {
                ...
                if (dataJson != null && dataJson.has("auth_code")) {
                    ...
                    if (!TextUtils.isEmpty(authCode)) {
                        Intent intent = new Intent(context, BageWebLoginConfirmActivity.class);
        ...
        EndpointManager.getInstance().setMethod("chow_reset_login_pwd_view", object -> {
            Intent intent = new Intent(context, BageResetLoginPwdActivity.class);
            intent.addFlags(FLAG_ACTIVITY_NEW_TASK);
            context.startActivity(intent);
            return null;
        });
        ...
        EndpointManager.getInstance().setMethod("show_web_login_desc", object -> {
            ...
            Intent intent = new Intent(activity, BageWebLoginActivity.class);
            intent.addFlags(FLAG_ACTIVITY_NEW_TASK);
            activity.startActivity(intent);
            return null;
        });
