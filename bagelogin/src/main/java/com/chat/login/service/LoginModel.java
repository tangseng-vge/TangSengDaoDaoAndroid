 ...
 * 登录model
 */
public class LoginModel extends BageBaseModel {
    private LoginModel() {
    }
    ...
    void loginApp(String name, String pwd, final ILoginListener iLoginListener) {
        ...
        deviceJson.put("device_id", BageConstants.getDeviceID());
        deviceJson.put("device_name", BageDeviceUtils.getInstance().getDeviceName());
        deviceJson.put("device_model", BageDeviceUtils.getInstance().getSystemModel());
    ...
    private void saveLoginInfo(UserInfoEntity userInfo) {
        ...
        BageConfig.getInstance().setToken(userInfo.token);
        if (!TextUtils.isEmpty(userInfo.im_token)) {
            BageConfig.getInstance().setImToken(userInfo.im_token);
        } else BageConfig.getInstance().setImToken(userInfo.token);
        BageConfig.getInstance().setUid(userInfo.uid);
        BageConfig.getInstance().setUserName(userInfo.name);
    }
}
