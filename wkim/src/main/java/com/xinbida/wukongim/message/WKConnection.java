        ...
        try {
            executor.execute(() -> {
                try {
                    ...
                    ConnectionClient newClient = new ConnectionClient(iNonBlockingConnection -> {
                        try {
                            if (!connectionPublishedLatch.await(2, TimeUnit.SECONDS)) {
                                WKLoggerUtils.getInstance().e(TAG, "TLS 连接对象发布超时");
                        ...
                        try {
                            if (!iNonBlockingConnection.isSecure()) {
                                WKLoggerUtils.getInstance().e(TAG, "TLS 握手未完成，拒绝发送 IM CONNECT 包");
                    ...
                    // Modified in TangSengDaoDao: establish TLS before sending any IM protocol data.
                    ...
                    INonBlockingConnection newConnection = new NonBlockingConnection(
                            ...
                            false,
                            5000,
                            WKClientSSLContext.create(connIp, connPort),
                            true
                    );
                    ...
                    WKLoggerUtils.getInstance().e(TAG, "TLS 上下文初始化失败: " + e.getMessage()
