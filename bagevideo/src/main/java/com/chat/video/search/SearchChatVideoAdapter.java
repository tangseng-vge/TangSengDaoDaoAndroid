    ...
    protected void convert(@NotNull BaseViewHolder baseViewHolder, SearchChatVideoEntity searchChatVideoEntity) {
        switch (searchChatVideoEntity.getItemType()) {
            ...
            case 0 -> {
                ...
                list.add(new PopupMenuItem(getContext().getString(R.string.bage_video_go_to_chat_item), R.mipmap.msg_message, () -> icLick.onClick(searchChatVideoEntity)));

                BageDialogUtils.getInstance().setViewLongClickPopup(baseViewHolder.getView(R.id.imageView), list);
            }
        }
    }
