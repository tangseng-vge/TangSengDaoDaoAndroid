    ...
    override fun convert(holder: BaseViewHolder, item: SearchEntity) {
        when (item.itemType) {
            ...
            0 -> {
                ...
                GlideUtils.getInstance().showImg(context, BageApiConfig.getShowUrl(item.videoModel.cover), imageView)
                val list: MutableList<PopupMenuItem> = ArrayList()
                list.add(
                    PopupMenuItem(context.getString(R.string.bage_video_go_to_chat_item),
