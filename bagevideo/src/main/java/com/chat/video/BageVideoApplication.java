 ...
 * 视频model
 */
public class BageVideoApplication {
    private BageVideoApplication() {
    }

    private static class VideoApplicationBinder {
        final static BageVideoApplication VIDEO = new BageVideoApplication();
    }

    public static BageVideoApplication getInstance() {
        return VideoApplicationBinder.VIDEO;
    }

    public void init(Context context) {
        AppModule appModule = BageBaseApplication.getInstance().getAppModuleWithSid("video");
        if (!BageBaseApplication.getInstance().appModuleIsInjection(appModule)) {
            return;
        }
        //添加消息item
        BageMsgItemViewManager.getInstance().addChatItemViewProvider(BageMsgContentType.Bage_VIDEO, new VideoItemProvider());
        // 注册长按菜单
        EndpointManager.getInstance().setMethod(EndpointCategory.msgConfig + BageContentType.Bage_VIDEO, object -> new MsgConfig(true));
...
//        });
        ...
        //添加聊天功能面板
        EndpointManager.getInstance().setMethod(EndpointCategory.chatFunction + "_recording", EndpointCategory.chatFunction, 99, object -> new ChatFunctionMenu("recording", R.mipmap.ic_send_camera, context.getString(R.string.bagevideo_recording), (iConversationContext) -> {
        ...
        EndpointManager.getInstance().setMethod(EndpointCategory.bageChatToolBar + "_album", EndpointCategory.bageChatToolBar, 98, object -> new ChatToolBarMenu("bage_chat_toolbar_camera", R.mipmap.icon_chat_toolbar_camera, -1, null, (isSelected, iConversationContext1) -> {
            if (isSelected) {
                BageVideoApplication.this.iConversationContext = iConversationContext1;
        ...
        // 搜索聊天图片
//        EndpointManager.getInstance().setMethod("str_search_chat_video", EndpointCategory.bageSearchChatContent, 95, object -> new SearchChatContentMenu(context.getString(R.string.bage_video), (channelID, channelType) -> {
...
//        }));
    ...
    void setRecordingResult(long second, String path, String videoPath, long size) {
        ...
        if (!TextUtils.isEmpty(videoPath)) {
            BageVideoContent videoMsgModel = new BageVideoContent();
            ...
            if (iConversationContext != null) {
                iConversationContext.sendMessage(new BageChatImageContent(path));
                iConversationContext = null;
            }
        }
    }
    ...
    private void startRecord(Context context) {
        ...
        if (endpoint != null) {
            ...
            if (isCalling) {
                BageToastUtils.getInstance().showToastNormal(context.getString(R.string.not_use_the_camera));
                return;
            }
        }
...
//        Integer telegramId = org.telegram.messenger.VideoConvertUtil.startVideoConvert(videoPath, BageConstants.videoDir + "test.mp4", new MediaController.ConvertorListener() {
//            @Override
...
//        VideoCompress.VideoCompressTask task = VideoCompress.compressVideoHigh(videoPath, BageConstants.videoDir + "test.mp4", new VideoCompress.CompressListener() {
//            @Override
//            public void onStart() {
//                //Start Compress
//                Log.e("开始压缩", "-->"+ BageTimeUtils.getInstance().getCurrentSeconds());
//            }
//
//            @Override
//            public void onSuccess() {
//                //Finish successfully
//                Log.e("开始压缩成功", "-->"+BageTimeUtils.getInstance().getCurrentSeconds());
//            }
//
//            @Override
//            public void onFail() {
