...
import com.bage.im.utils.BigTypeUtils;
...
public class BageRead {
    ...
    public int readPacketType() throws IOException {
        ...
        return BageTypeUtils.getInstance().getHeight4(header[0]);
    }

    public int readRemainingLength() {
        return BageTypeUtils.getInstance().bytes2Length(inputStream);
    }
    ...
    public String readString() throws IOException {
        ...
        if (read == -1) return "";
        return BageTypeUtils.getInstance().bytesToString(strByte);
    }
    ...
    public String readPayload() throws IOException {
        ...
        if (read == -1) return "";
        return BageTypeUtils.getInstance().bytesToString(payload);
    }
}
