...
import com.bage.im.interfaces.INewReminderListener;
import com.bage.im.message.BageRead;
...
public class ReminderManager extends BaseManager {
    ...
    private ConcurrentHashMap<String, ArrayList<BageReminder>> reminderList;
    ...
    private void setNewReminders(List<BageReminder> list) {
     ...
     * @return BageReminder
     */
    public BageReminder getReminder(String channelID, byte channelType, int type) {
        List<BageReminder> list = getReminders(channelID, channelType);
        BageReminder bageReminder = null;
        for (int i = 0, size = list.size(); i < size; i++) {
            if (list.get(i).type == type) {
                bageReminder = list.get(i);
                break;
            }
        }
        return bageReminder;
     ...
     * @param channelID   频道ID
     * @param channelType 频道类型
     * @return List<BageReminder>
     */
    public List<BageReminder> getReminders(String channelID, byte channelType) {
        // 先查内存缓存
        String key = getCacheKey(channelID, channelType);
        if (reminderList != null && reminderList.containsKey(key)) {
            ArrayList<BageReminder> cached = reminderList.get(key);
            if (cached != null) {
        ...
        // 缓存未命中，查询数据库
        List<BageReminder> list = ReminderDBManager.getInstance().queryWithChannelAndDone(channelID, channelType, 0);
     ...
     * 缓存 Reminder 列表
     */
    private void cacheReminders(String channelID, byte channelType, List<BageReminder> list) {
    ...
    public List<BageReminder> getRemindersWithType(String channelID, byte channelType, int type) {
        return ReminderDBManager.getInstance().queryWithChannelAndTypeAndDone(channelID, channelType, type, 0);
    }

    public void saveOrUpdateReminders(List<BageReminder> reminderList) {

        if (BageCommonUtils.isNotEmpty(reminderList)) {
     ...
     * 更新内存缓存
     */
    private void updateMemoryCache(List<BageReminder> reminders) {
        if (this.reminderList == null) {
            this.reminderList = new ConcurrentHashMap<>();
        }
        
        for (BageReminder reminder : reminders) {
     ...
     * 更新或添加 Reminder 到缓存
     */
    private void updateOrAddReminderToCache(String key, BageReminder newReminder) {
        // 获取该频道的缓存列表
        ArrayList<BageReminder> list = this.reminderList.get(key);
     ...
     * 从缓存中移除指定的 Reminder
     */
    private void removeReminderFromCache(String key, BageReminder reminderToRemove) {
