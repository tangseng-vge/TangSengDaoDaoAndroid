 ...
 * 频道成员实体
 */
public class BageChannelMember implements Parcelable {
    //自增ID
    public long id;
    //频道id
    ...
    protected BageChannelMember(Parcel in) {
        id = in.readLong();
        status = in.readInt();
        ...
        extraMap = BageCommonUtils.str2HashMap(extraStr);
    }
    ...
    public static final Creator<BageChannelMember> CREATOR = new Creator<BageChannelMember>() {
        @Override
        public BageChannelMember createFromParcel(Parcel in) {
            return new BageChannelMember(in);
        }

        @Override
        public BageChannelMember[] newArray(int size) {
            return new BageChannelMember[size];
        }
    };
}
