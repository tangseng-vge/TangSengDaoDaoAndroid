 ...
 * 频道实体
 */
public class BageChannel implements Parcelable {
    //自增ID
    public long id;
    //频道ID
...
//    public int mainDeviceFlag;
    ...
    public BageChannel(String channelID, byte channelType) {
    ...
    protected BageChannel(Parcel in) {
        id = in.readLong();
        ...
        localExtra = BageCommonUtils.str2HashMap(localStr);
        String remoteStr = in.readString();
        remoteExtraMap = BageCommonUtils.str2HashMap(remoteStr);
    }
    ...
    public static final Creator<BageChannel> CREATOR = new Creator<BageChannel>() {
        @Override
        public BageChannel createFromParcel(Parcel in) {
            return new BageChannel(in);
        }

        @Override
        public BageChannel[] newArray(int size) {
            return new BageChannel[size];
        }
    };
