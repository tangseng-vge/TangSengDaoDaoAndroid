...
class StickerDBManager private constructor() {
    ...
    fun getUserCustomSticker(): ArrayList<Sticker> {
        ...
        val cursor: Cursor = BageBaseApplication.getInstance().dbHelper.rawQuery(sql, null)
            ?: return list
        run {
            cursor.moveToFirst()
    ...
    fun getCustomerWithPath(path: String): Sticker? {
        ...
        val cursor: Cursor = BageBaseApplication.getInstance().dbHelper.rawQuery(sql) ?: return null
        cursor.moveToFirst()
        var sticker = Sticker()
    ...
    private fun clearCategory() {
        BageBaseApplication.getInstance().dbHelper
    ...
    fun addUserStickerCategory(list: List<StickerCategory>?) {
        if (list!!.isNotEmpty()) {
            try {
                clearCategory()
                BageBaseApplication.getInstance().dbHelper.db.beginTransaction()
                for (category in list) {
                    insertCategory(category)
                }
                BageBaseApplication.getInstance().dbHelper.db.setTransactionSuccessful()
            } catch (e: Exception) {
            } finally {
                if (BageBaseApplication.getInstance().dbHelper.db.inTransaction()) {
                    BageBaseApplication.getInstance().dbHelper.db.endTransaction()
                }
            }
        }
    }
    ...
    fun addSticker(list: List<Sticker>?) {
        if (list!!.isNotEmpty()) {
            try {
                ...
                BageBaseApplication.getInstance().dbHelper.db.beginTransaction()
                for (cv in cvList) {
                    BageBaseApplication.getInstance().dbHelper.insert("sticker", cv)
                }
                BageBaseApplication.getInstance().dbHelper.db.setTransactionSuccessful()
            } catch (_: Exception) {
            } finally {
    ...
    fun deleteStickerCategory(category: String): Boolean {
        ...
        return BageBaseApplication.getInstance().dbHelper
...
//            val num = getCustomStickerMaxSortNum()
    ...
    fun insertUserCustomSticker(sticker: Sticker?) {
        ...
        BageBaseApplication.getInstance().dbHelper.insert(customSticker, cv)
    }
    ...
    private fun insertCategory(category: StickerCategory?) {
        ...
        BageBaseApplication.getInstance().dbHelper.insert(userStickerCategory, cv)
    }
    ...
    private fun serializeSticker(cursor: Cursor): Sticker {
        val sticker = Sticker()
        sticker.path = BageCursor.readString(cursor, "path")
        sticker.category = BageCursor.readString(cursor, "category")
        sticker.width = BageCursor.readInt(cursor, "width")
        sticker.height = BageCursor.readInt(cursor, "height")
        sticker.title = BageCursor.readString(cursor, "title")
        sticker.placeholder = BageCursor.readString(cursor, "placeholder")
        sticker.format = BageCursor.readString(cursor, "format")
        sticker.searchable_word = BageCursor.readString(cursor, "searchable_word")
        return sticker
    }
