 ...
 * 表情model
 */
class StickerModel : BageBaseModel() {
...
//        val list = getSticker(BageConfig.getInstance().uid)
        if (list.isNotEmpty()) {
...
//                saveStickerCategory(result)
                    ...
                    override fun onResult(
                        ...
                        if (code == HttpResponseCode.success.toInt() && stickerDetail!!.list.isNotEmpty()) {
                            for (sticker in stickerDetail.list) {
                                ...
                                val filePath = BageStickerApplication.instance.stickerDirPath + path
                                val file = File(filePath)
                                if (!file.exists()) {
                                    BageDownloader.instance.download(
                                        BageApiConfig.getShowUrl(sticker.path),
                                        filePath,
                                        object :
                                            BageProgressManager.IProgress {
    ...
    private fun saveStickerCategory(list: List<StickerCategory>) {
        ...
        BageSharedPreferencesUtil.getInstance()
            .putSP(BageConfig.getInstance().uid + "_sticker_category_list", jsonArray.toString())
    }
...
//            }
//        }
//        val key = String.format("%s_%s_stickers", BageConfig.getInstance().uid, category)
...
//    }
    ...
    fun getStickerUploadURL(iStickerUploadURLListener: IStickerUploadURLListener) {
        request(
            createService(ApiService::class.java).getUploadFileUrl(BageApiConfig.baseUrl + "file/upload?type=sticker"),
    ...
    fun downloadCustomerSticker(list: List<Sticker>) {
        for (sticker in list) {
            if (TextUtils.isEmpty(sticker.format)) {
                ...
                val url = BageApiConfig.getShowUrl(sticker.path)
    ...
    fun download(url: String, savePath: String) {
        ...
        BageDownloader.instance.download(url, savePath, object : BageProgressManager.IProgress {
            override fun onProgress(tag: Any?, progress: Int) {
            }
