...
public class EmojiView extends FrameLayout {
    ...
    public EmojiView(@NonNull Context context) {
        ...
        String ids = BageSharedPreferencesUtil.getInstance().getSPWithUID(Const.commonUsedEmojisKey);
...
//        });
        ...
        stickersGridAdapter.setOnItemChildClickListener((adapter12, view, position) -> {
            ...
            lastClickTime = BageTimeUtils.getInstance().getCurrentMills();
            ...
            if (stickerUI.getItemType() == 1) {
                ...
                BageStickerApplication.Companion.getInstance().sendMsg(vectorSticker);
            } else if (stickerUI.getItemType() == 2) {
                BageGifContent gifContent = new BageGifContent();
                ...
                BageStickerApplication.Companion.getInstance().sendMsg(gifContent);
//                BageStickerApplication.Companion.getInstance().iConversationContext.sendMessage(gifContent);
            }
        });
...
//            sickerTabAdapter.notifyDataSetChanged();
        ...
        EndpointManager.getInstance().setMethod("", EndpointCategory.bageRefreshStickerCategory, object -> {
            getStickers(context);
//            sickerTabAdapter.setList(stickerTabList);
//            sickerTabAdapter.notifyDataSetChanged();
            return null;
    ...
    private void emojiClick(EmojiEntity emojiEntity) {
        ...
        String usedIndexList = BageSharedPreferencesUtil.getInstance().getSPWithUID(Const.commonUsedEmojisKey);
        ...
        BageSharedPreferencesUtil.getInstance().putSPWithUID(Const.commonUsedEmojisKey, tempIndexList);
    }
