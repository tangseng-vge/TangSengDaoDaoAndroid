 ...
 * 自定义表情
 */
class CustomStickerActivity : BageBaseActivity<ActCustomStickerLayoutBinding>() {
    ...
    override fun initView() {
        ...
        bageVBinding.recyclerView.layoutManager =
            StaggeredGridLayoutManager(5, StaggeredGridLayoutManager.VERTICAL)
        bageVBinding.recyclerView.adapter = adapter

        ViewCompat.setOnApplyWindowInsetsListener(bageVBinding.root) { v: View, insets: WindowInsetsCompat ->
            val systemBars =
    ...
    override fun initListener() {
        ...
        bageVBinding.removeToTopTv.setOnClickListener {
        ...
        bageVBinding.deleteTv.setOnClickListener {
            if (isEnable) {
    ...
    private fun getStickers() {
        StickerModel().getUserCustomSticker(object : StickerModel.IStickersListener {
            override fun onResult(code: Int, msg: String, list: List<Sticker>) {
                ...
                if (BageReader.isEmpty(list) || list.size == 1) {
                    bageVBinding.removeToTopTv.isClickable = false
                    bageVBinding.removeToTopTv.alpha = 0.2f
                }
                val sticker = Sticker()
                sticker.path = "addCustom"
                adapter.addData(0, sticker)
    ...
    private fun checkBottomViewStatus() {
        ...
        bageVBinding.removeToTopTv.isClickable = isEnable
        bageVBinding.deleteTv.isClickable = isEnable
        if (isEnable) {
            bageVBinding.removeToTopTv.alpha = if (count >= 1) 1f else 0.2f
            bageVBinding.deleteTv.alpha = 1f
        } else {
            bageVBinding.removeToTopTv.alpha = 0.2f
            bageVBinding.deleteTv.alpha = 0.2f
        }
    }
