 ...
 * sticker view
 */
public class StickerView extends FrameLayout implements BageProgressManager.IProgress {
...
//        imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
    ...
    public synchronized void showSticker(String url, String placeholder, int size, boolean isLoopPlay, boolean isPlay) {
        ...
        this.size = size;
        StickerView.this.tag = BageApiConfig.getShowUrl(url);

        if (isRasterSticker(url)) {
            imageView.setTag(url);
            loadRasterImage(BageApiConfig.getShowUrl(url));
        } else {
...
//            this.size = size;
            ...
            BageStickerApplication.Companion.getInstance().getDispatchQueuePool().execute(() -> {
                String fileName = String.format("%s", url.replaceAll("/", "_"));
                String fileDir = BageStickerApplication.Companion.getInstance().getStickerDirPath();
                ...
                if (file.exists()) {
                    ...
                    BageDownloader.Companion.getInstance().download(BageApiConfig.getShowUrl(url), file1.getAbsolutePath(), this);
                }
            });
        }
    }
    ...
    public void showSticker(RLottieImageView aXrLottieImageView, boolean isLoopPlay, File file, int size, boolean isPlay) {
            ...
            if (!jsonFile.exists()) {
                String path = BageFileUtils.getInstance().uncompressSticker(BageFileUtils.getInstance().file2byte(file), fileName);
