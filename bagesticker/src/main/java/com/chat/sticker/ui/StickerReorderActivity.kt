 ...
 * 重新排序
 */
class StickerReorderActivity : BageBaseActivity<ActStickerReorderLayoutBinding>() {
            ...
            if (code == HttpResponseCode.success.toInt()) {
                StickerModel().fetchCategoryList(object : StickerModel.IStickerCategoryListener {
                    override fun onResult(code: Int, msg: String, list: List<StickerCategory>) {
                        ...
                        if (code == HttpResponseCode.success.toInt()) {
                            val menus: List<StickerCategoryRefreshMenu> = EndpointManager.getInstance().invokes(EndpointCategory.bageRefreshStickerCategory, null)
                            if (menus.isNotEmpty()) {
