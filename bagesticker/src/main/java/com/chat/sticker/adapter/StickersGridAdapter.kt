    ...
    override fun convert(holder: BaseViewHolder, item: StickerUI) {
        if (item.viewType == 0) {
            ...
            stickerView.showSticker(
                ...
                BageStickerApplication.instance.stickerGridSize,
                true
            )
        } else {
            val stickerView = holder.getView<StickerView>(R.id.stickerView)
            val imageView = stickerView.imageView
            imageView.layoutParams.width = BageStickerApplication.instance.stickerGridSize
            imageView.layoutParams.height = BageStickerApplication.instance.stickerGridSize
            //han
            stickerView.showSticker(
                item.sticker!!.path,
                item.sticker!!.placeholder,
...
//                    showURL = BageApiConfig.getShowUrl(item.sticker!!.path)
//                    Glide.with(context)
//                        .load(showURL)
