 ...
 * 表情商店
 */
class BageStickerApplication {

    private object SingletonInstance {
        val INSTANCE = BageStickerApplication()
    }
...
//            initStickerGridData();
        ...
        val appModule = BageBaseApplication.getInstance().getAppModuleWithSid("sticker")
        if (!BageBaseApplication.getInstance().appModuleIsInjection(appModule)) return

        val stickerDir = Objects.requireNonNull<File>(
            BageBaseApplication.getInstance().context.getExternalFilesDir("stickerCache")
        ).absolutePath + "/"
        BageFileUtils.getInstance().createFileDir(stickerDir)
        stickerDirPath =
            Objects.requireNonNull<File>(
                BageBaseApplication.getInstance().context.getExternalFilesDir(
                    "bageStickers"
                )
            ).absolutePath + "/"
        BageFileUtils.getInstance().createFileDir(stickerDirPath)
        // 清空缓存
        Thread { BageFileUtils.delFileOrFolder(File(stickerDir)) }.start()

        //注册消息item
        BageIM.getInstance().msgManager.registerContentMsg(BageGifContent::class.java)
        BageIM.getInstance().msgManager.registerContentMsg(EmojiContent::class.java)
        BageIM.getInstance().msgManager.registerContentMsg(StickerContent::class.java)
        // 注册长按类型
        EndpointManager.getInstance()
            .setMethod(EndpointCategory.msgConfig + BageContentType.Bage_VECTOR_STICKER) {
                MsgConfig(
                    true
                )
            }
        EndpointManager.getInstance()
            .setMethod(EndpointCategory.msgConfig + BageContentType.Bage_EMOJI_STICKER) { MsgConfig(true) }
        EndpointManager.getInstance()
            .setMethod(EndpointCategory.msgConfig + BageContentType.Bage_GIF) { MsgConfig(true) }
        // 注册消息item
        BageMsgItemViewManager.getInstance()
            .addChatItemViewProvider(BageContentType.Bage_GIF, GifProvider())
        BageMsgItemViewManager.getInstance()
            .addChatItemViewProvider(
                BageContentType.Bage_EMOJI_STICKER,
                EmojiProvider()
            )
        BageMsgItemViewManager.getInstance()
            .addChatItemViewProvider(
                BageContentType.Bage_VECTOR_STICKER,
                StickerProvider()
            )
        ...
        EndpointManager.getInstance()
            .setMethod("", EndpointCategory.bageChatPopupItem, 29, object : EndpointHandler {
                override fun invoke(`object`: Any?): Any? {
                    val mMsg = `object` as BageMsg
                    if (mMsg.type == BageContentType.Bage_VECTOR_STICKER) {
                        return ChatItemPopupMenu(
                            R.mipmap.msg_sticker,
                            BageBaseApplication.getInstance().context.getString(R.string.show_collections),
        ...
        EndpointManager.getInstance()
            .setMethod("", EndpointCategory.bageChatPopupItem, 30, object : EndpointHandler {
                override fun invoke(`object`: Any?): Any? {
                    val msg = `object` as BageMsg
                    when (msg.type) {
                        BageContentType.Bage_VECTOR_STICKER, BageContentType.Bage_GIF -> {
                            val path: String = if (msg.type == BageContentType.Bage_GIF) {
                                val gifModel =
                                    msg.baseContentMsgModel as BageGifContent
                                gifModel.url
                            } else {
                                val stickerModel =
                            ...
                            var isAdd = true
                            var text =
                                BageBaseApplication.getInstance().context.getString(R.string.str_sticker_add)
                            if (sticker != null && !TextUtils.isEmpty(sticker.path) && sticker.path == path) {
                                resourceID = R.mipmap.msg_unfave
                                text =
                                    BageBaseApplication.getInstance().context.getString(R.string.delete_from_favorites)
                                isAdd = false
                            }
                            return ChatItemPopupMenu(
                                resourceID,
                                ...
                                object : ChatItemPopupMenu.IPopupItemClick {
                                    override fun onClick(
                                        ...
                                        if (isAdd) {
                                            ...
                                            var width = 0
                                            var height = 0
                                            if (mMsg.type == BageContentType.Bage_GIF) {
                                                val gifModel =
                                                    mMsg.baseContentMsgModel as BageGifContent
                                                url = gifModel.url
                                                width = gifModel.width
                                                height = gifModel.height
                                            } else if (mMsg.type == BageContentType.Bage_VECTOR_STICKER) {
                                                val stickerModel =
                                                    mMsg.baseContentMsgModel as StickerContent
                                            ...
                                            StickerModel().addSticker(
                                                ...
                                                if (code1 == HttpResponseCode.success.toInt()) {
                                                    ...
                                                    Snackbar.make(
                                                        ...
                                                        .show()
                                                } else BageToastUtils.getInstance()
                        ...
                        BageContentType.Bage_EMOJI_STICKER -> {
                            return ChatItemPopupMenu(
                                R.mipmap.msg_copy,
                                BageBaseApplication.getInstance().context.getString(R.string.copy),
                                object : ChatItemPopupMenu.IPopupItemClick {
                                    override fun onClick(
                                        mMsg: BageMsg,
                                        iConversationContext: IConversationContext
...
//        initStickerGridData();
        ...
        // 注册聊天工具栏
        EndpointManager.getInstance().setMethod(
            EndpointCategory.bageChatToolBar + "_sticker",
            EndpointCategory.bageChatToolBar,
            100
        ) { `object` ->
                        ...
                        override fun onResult(code: Int, msg: String, list: List<Sticker>) {
                            ...
                            if (list.isNotEmpty()) {
                                ...
                                if (currentRecyclerView.visibility == GONE) {
                                    ...
                                    var h = BageConstants.getKeyboardHeight()
                                    if (h == 0) {
    ...
    fun sendMsg(message: BageMessageContent) {
...
//    }
    ...
    private fun fetchSticker() {
        ...
        if (!TextUtils.isEmpty(BageConfig.getInstance().token)) {
            val isRefreshStickerCategory =
                BageSharedPreferencesUtil.getInstance().getBooleanWithUID("isRefreshStickerCategory")
    ...
    fun getStickerView(conversationContext: IConversationContext): View {
        val parentView = FrameLayout(conversationContext.chatActivity)
    ...
    fun forwardMessage(context: Context, mMsg: BageMsg) {
        var mMessageContent =
