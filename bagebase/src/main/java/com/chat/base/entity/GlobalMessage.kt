...
class GlobalMessage {
    ...
    private var messageContent: BageMessageContent? = null

    fun getMessageModel(): BageMessageContent? {
        if (messageContent == null) {
            val jsonObject = JSONObject(payload)
            messageContent = BageIM.getInstance().msgManager.getMsgContentModel(jsonObject)
        }
    ...
    fun getHtmlWithField(field: String): String {
        ...
        if (content is String && !TextUtils.isEmpty(content)) {
            return BageBaseApplication.getInstance().application.getString(R.string.last_message_file) + " " + content.replace(
                "<mark>",
                "<font color=#f65835>"
            )
                .replace("</mark>", "</font>")
