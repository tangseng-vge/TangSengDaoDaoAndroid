 ...
 * toast
 */
public class BageToastUtils {
    private BageToastUtils() {
    }

    private static class ToastUtilsBinder {
        private static final BageToastUtils utils = new BageToastUtils();
    }

    public static BageToastUtils getInstance() {
    ...
    private void showToast(String msg, int type) {
        Toast toast = Toast.makeText(BageBaseApplication.getInstance().getContext(), msg, Toast.LENGTH_SHORT);
    ...
    private View getToastView(String content, int type) {
        View view = LayoutInflater.from(BageBaseApplication.getInstance().getContext()).inflate(R.layout.bage_toast_layout, null);
