 ...
 * 系统状态栏
 */
public class BageStatusBarUtils {

    public static boolean supportTransparentStatusBar() {
        return BageOSUtils.isMiui()
                || BageOSUtils.isFlyme()
                || (BageOSUtils.isOppo() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)
    ...
    public static void transparentStatusBar(Window window) {
        if (BageOSUtils.isMiui() || BageOSUtils.isFlyme()) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                transparentStatusBarAbove21(window);
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            }
        } else if ((BageOSUtils.isOppo() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)) {
            transparentStatusBarAbove21(window);
