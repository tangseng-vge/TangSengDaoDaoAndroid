 ...
 * 通用弹框
 */
public class BageDialogUtils {
    private BageDialogUtils() {
    }

    private static class DialogUtilsBinder {
        static BageDialogUtils utils = new BageDialogUtils();
    }

    public static BageDialogUtils getInstance() {
...
//            window.decorView.setBackgroundColor(Color.RED)
        ...
        sureBtn.setOnClickListener(view1 -> {
            DownloadApkUtils.getInstance().downloadAPK(BageBaseApplication.getInstance().getContext(),
                    versionEntity.app_version,
                    BageApiConfig.getShowUrl(versionEntity.download_url));
            alertDialog.dismiss();
...
//            scrimPopupWindow.dimBehind();
    ...
    public BasePopupView showImagePopup(Context context, BageMsg msg, List<Object> tempImgList, List<ImageView> imgList, ImageView imageView, int index, List<ImagePopupBottomSheetItem> list, final CustomImageViewerPopup.IImgPopupMenu iImgPopupMenu, final IImagePopupListener iImagePopupListener) {
