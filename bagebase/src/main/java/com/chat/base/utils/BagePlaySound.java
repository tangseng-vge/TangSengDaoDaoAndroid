...
public class BagePlaySound {
    ...
    private BagePlaySound() {
    }

    private static class PlaySoundBinder {
        static final BagePlaySound play = new BagePlaySound();
    }

    public static BagePlaySound getInstance() {
    ...
    public void playRecordMsg(int playID) {
        try {
            ...
            if (soundRecord == 0 && !soundRecordLoaded) {
                ...
                soundRecord = soundPool.load(BageBaseApplication.getInstance().getContext(), playID, 1);
            }
            if (soundRecord != 0) {
                try {
    ...
    public void playOutMsg(int playID) {
        try {
            ...
            if (soundOut == 0 && !soundOutLoaded) {
                soundOutLoaded = true;
                soundOut = soundPool.load(BageBaseApplication.getInstance().getContext(), playID, 1);
            }
            if (soundOut != 0) {
                try {
    ...
    public void playInMsg(int playID) {
        try {
            ...
            if (soundIn == 0 && !soundInLoaded) {
                soundInLoaded = true;
                soundIn = soundPool.load(BageBaseApplication.getInstance().getContext(), playID, 1);
            }
            if (soundIn != 0) {
                try {
