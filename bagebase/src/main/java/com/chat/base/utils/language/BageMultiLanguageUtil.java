 ...
 * 多语言切换的帮助类
 */
public class BageMultiLanguageUtil {
    ...
    private static class MultiLanguageUtilBinder {
        private final static BageMultiLanguageUtil util = new BageMultiLanguageUtil();
    }

    public static BageMultiLanguageUtil getInstance() {
    ...
    private Locale getLanguageLocale() {
        ...
        int languageType = BageSharedPreferencesUtil.getInstance().getInt(BageMultiLanguageUtil.SAVE_LANGUAGE, 0);
    ...
    public void updateLanguage(int languageType) {
        BageSharedPreferencesUtil.getInstance().putInt(BageMultiLanguageUtil.SAVE_LANGUAGE, languageType);
        BageMultiLanguageUtil.getInstance().setConfiguration();
    }

    public String getLanguageName(Context context) {
        int languageType = BageSharedPreferencesUtil.getInstance().getInt(BageMultiLanguageUtil.SAVE_LANGUAGE, BageLanguageType.LANGUAGE_FOLLOW_SYSTEM);
    ...
    public int getLanguageType() {
        int languageType = BageSharedPreferencesUtil.getInstance().getInt(BageMultiLanguageUtil.SAVE_LANGUAGE, BageLanguageType.LANGUAGE_FOLLOW_SYSTEM);
        if (languageType == BageLanguageType.LANGUAGE_CHINESE_SIMPLIFIED) {
            return BageLanguageType.LANGUAGE_CHINESE_SIMPLIFIED;
        } else if (languageType == BageLanguageType.LANGUAGE_CHINESE_TRADITIONAL) {
            return BageLanguageType.LANGUAGE_CHINESE_TRADITIONAL;
        } else if (languageType == BageLanguageType.LANGUAGE_FOLLOW_SYSTEM) {
            return BageLanguageType.LANGUAGE_FOLLOW_SYSTEM;
        }
