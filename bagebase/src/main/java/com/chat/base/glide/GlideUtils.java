...
public class GlideUtils {
    ...
    public void showAvatarImg(Context mContext, String channelID, byte channelType, String key, ImageView imageView) {
        String url = BageApiConfig.getShowAvatar(channelID, channelType);
...
// 在ImageView上显示圆角头像
    ...
    public void chooseIMG(Activity activity, int maxSelectNum, boolean isCamera, ChooseMimeType mimeType, boolean isWithSelectVideoImage, boolean isOriginalControl, final ISelectBack iSelectBack) {
        if (isCamera) BageBaseApplication.getInstance().disconnect = false;
...
//                .isCompress(true)
                ...
                .isGif(true).forResult(new OnResultCallbackListener<>() {
                    ...
                    public void onResult(ArrayList<LocalMedia> result) {
                        ...
                        for (LocalMedia media : result) {
                                ...
                                if (PictureMimeType.isContent(chooseResult.path)) {
                                    ...
                                    if (!TextUtils.isEmpty(realPath)) {
                                        ...
                                        if (!TextUtils.isEmpty(fallbackPath)) {
                                            ...
                                            BageLogUtils.e("Image path conversion failed, using original: " + chooseResult.path);
                                        }
                                    }
                                }
                            ...
                            BageLogUtils.e("Selected path: " + chooseResult.path + ", model: " + chooseResult.model);
    ...
    private String getRealPathFromUri(Context context, Uri contentUri) {
        ...
        try {
            ...
            BageLogUtils.e("getRealPathFromUri error: " + e.getMessage());
        } finally {
            if (cursor != null) {
                cursor.close();
            }
        }
        // 如果上述方法失败，尝试使用 BageFileUtils 的方法
        try {
            return BageFileUtils.getInstance().getPath(contentUri);
        } catch (Exception e) {
            BageLogUtils.e("BageFileUtils.getPath error: " + e.getMessage());
        }
        return null;
    }
    ...
    public void compressImg(Context context, List<String> paths, final ICompressListener iCompressListener) {
        ...
        Luban.with(context)
                .load(paths)
                .ignoreBy(100)
                .setTargetDir(BageConstants.imageDir)
