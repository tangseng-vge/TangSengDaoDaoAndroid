 ...
 * 基础类
 */
public abstract class BageBaseActivity<BageVBinding extends ViewBinding> extends SwipeBackActivity {
    private RadialProgressView titleRightLoadingIv;
    private View titleRightLayout;
    private Button rightBtn;
    protected LoadingPopupView loadingPopup;
    protected View view;
    protected BageVBinding bageVBinding;
...
//        }
        boolean na = BageStatusBarUtils.isNavigationBarExist(this);
//        if (na) {
//            getWindow().getDecorView().findViewById(android.R.id.content).setPadding(0, 0, 0, BageStatusBarUtils.getNavigationBarHeight(this));
//        }
        BageMultiLanguageUtil.getInstance().setConfiguration();
//        adaptTheme(false);
        bageVBinding = getViewBinding();
        setContentView(bageVBinding.getRoot());
        initSwipeBackFinish();
        initPresenter();
...
//        setStatusBarColor();
    ...
    public void setContentView(View view) {
        ...
        if (statusBarView != null) {
            ...
            params.height = BageStatusBarUtils.getStatusBarHeight(this);
...
//        BageStatusBarUtils.transparentStatusBar(window);
//        BageStatusBarUtils.setStatusBarColor(window, ContextCompat.getColor(this, color), 0);
//        if (!Theme.getDarkModeStatus(this))
//            BageStatusBarUtils.setDarkMode(window);
//        else BageStatusBarUtils.setLightMode(window);
//    }
    ...
    protected abstract BageVBinding getViewBinding();
    ...
    //显示一个弹框
    protected void showDialog(String content, BageDialogUtils.IClickListener iClickListener) {
        BageDialogUtils.getInstance().showDialog(this, "", content, true, "", "", 0, 0, iClickListener);
    }

    protected void showSingleBtnDialog(String content) {
        BageDialogUtils.getInstance().showSingleBtnDialog(this, "", content, "", null);
    }

    protected void showToast(String content) {
        BageToastUtils.getInstance().showToastFail(content);
    }
...
//        titleRightIvLeft.setOnClickListener(v -> rightLeftLayoutClick());
    ...
    protected void attachBaseContext(Context newBase) {
        super.attachBaseContext(BageMultiLanguageUtil.getInstance().attachBaseContext(newBase));
    }
}
