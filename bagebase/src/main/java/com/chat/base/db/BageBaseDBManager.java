 ...
 * 数据库管理
 */
public class BageBaseDBManager {
    private BageBaseDBManager() {
    }

    private static class BaseDbManagerBinder {
        final static BageBaseDBManager baseDb = new BageBaseDBManager();
    }

    public static BageBaseDBManager getInstance() {
        return BaseDbManagerBinder.baseDb;
    }

    void onUpgrade(SQLiteDatabase db) {
        String loginUID = BageConfig.getInstance().getUid();
        long maxIndex = BageSharedPreferencesUtil.getInstance().getLong(loginUID + "_bage_db_upgrade_index");
        LinkedHashMap<Long, List<String>> map = getExecSQL();
        long tempIndex = maxIndex;
        for (Long key : map.keySet()) {
            List<String> list = map.get(key);
            if (key > maxIndex && BageReader.isNotEmpty(list)) {
                for (String sql : list) {
        ...
        BageSharedPreferencesUtil.getInstance().putLong(loginUID + "_bage_db_upgrade_index", tempIndex);
    }

    private LinkedHashMap<Long, List<String>> getExecSQL() {
        LinkedHashMap<Long, List<String>> sqlList = new LinkedHashMap<>();

        AssetManager assetManager = BageBaseApplication.getInstance().getContext().getAssets();
        if (assetManager != null) {
            try {
                List<DBMenu> DBMenuList = EndpointManager.getInstance().invokes(EndpointCategory.bageDBMenus, null);
                if (DBMenuList == null) DBMenuList = new ArrayList<>();
                DBMenuList.add(new DBMenu("bagebase_sql"));
